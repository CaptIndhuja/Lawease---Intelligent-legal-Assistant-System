...
export default function LeftPanel({ onThemeToggle, theme }) {
  ...
  async function handleFile(e) {
    ...
    setSelectedFile(f);   // Store selected file

    const fd = new FormData();
    fd.append('file', f);

    try {
      const r = await axios.post('http://127.0.0.1:8000/upload-document', fd);
  ...
  function deleteConversation(index) {
    ...
    setHistory(updated);
    localStorage.setItem('lawease_history', JSON.stringify(updated));
  }

  return (
      <div
    className="card p-4 shadow-lg border border-blue/10 no-scrollbar"
    style={{
      height: "100vh",
      overflowY: "auto"
    }}
  ...
  <button className='btn w-full' onClick={clearHistory}>Clear Conversation History</button>
    ...
    <div className="flex flex-col flex-1 mt-6">
      ...
      <div className="mt-2 space-y-2 flex-1 overflow-y-auto pr-1">
        ...
        {history.map((h, idx) => {
          const firstUserMsg = h.messages?.find(m => m.from === "user")?.text || "Conversation";
          const preview = firstUserMsg.split(" ").slice(0, 6).join(" ") + "...";

          return (
            <div
              key={idx}
              className="group relative p-2 bg-[#1a2340] hover:bg-[#263056] rounded cursor-pointer transition flex justify-between items-center"
            ...
            >
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300 font-bold px-2 transition absolute right-1 top-1"
              >
                ✕
              </button>
